...
{
    // Register Natives
    CreateNative("ControlUberCharger1",Native_ControlUberCharger);
    CreateNative("SetUberCharger1",Native_SetUberCharger);
    RegPluginLibrary("ubercharger1");
...
{
    g_IsUberchargerOn = CreateConVar("sm_ubercharger1","1","Enable/Disable ubercharger",0);
    g_ChargeAmount = CreateConVar("sm_ubercharger_charge_amount1", "0.35", "Sets the amount of uber charge to add time for medigun.",
                                  0, true, 0.0, true, 1.0);

    g_ChargeTimer = CreateConVar("sm_ubercharger_charge_timer1", "1.0", "Sets the time interval for medigun",0);

    g_EnableBeacon = CreateConVar("sm_ubercharger_beacon1","1","Enable/Disable ubercharger beacon",0);
    g_BeaconTimer = CreateConVar("sm_ubercharger_beacon_timer1","3.0","Sets the time interval of beacons for ubercharger",0);
    g_BeaconRadius = CreateConVar("sm_ubercharger_beacon_radius1", "375", "Sets the radius for medic enhancer beacon's light rings.",
                                  0, true, 50.0, true, 1500.0);

    g_EnablePing = CreateConVar("sm_ubercharger_ping1","1","Enable/Disable ubercharger ping",0);
    g_PingTimer = CreateConVar("sm_ubercharger_ping_timer1", "12.0", "Sets the time interval of pings for ubercharger.",0);

    // Execute the config file
    AutoExecConfig(true, "sm_ubercharger_beacon_timer1");

    CreateConVar("sm_tf_ubercharger1", PL_VERSION, "TF2 Medic Uber Charger Version", 0|FCVAR_SPONLY|FCVAR_REPLICATED|FCVAR_NOTIFY);
}
            ...
            if (IsClientInGame(client) && IsPlayerAlive(client))
            {
                ...
                if (team >= 2 && team <= 3)
                {
                    if (TF2_GetPlayerClass(client) == TFClass_Medic)
                    {
                        ...
                        {
                            new cond = TF2_IsPlayerInCondition(client, TFCond_Ubercharged);

                            //PrintToChat(client, "%i",cond);
